<x-app-layout>
@section('title', 'Inquire About ' . $classified->title)
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Inquire about {{ $classified->title }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        @php $thumb = $classified->media->first(); @endphp
                        @if ($thumb)
                            <img src="{{ $thumb->url }}" alt="{{ $classified->title }}"
                                class="h-20 w-20 rounded-lg object-cover flex-shrink-0">
                        @endif
                        <div>
                            <h3 class="text-lg font-bold text-orange-600 dark:text-orange-400">{{ $classified->title }}</h3>
                            <p class="text-2xl font-bold text-gray-800 dark:text-gray-100">${{ number_format($classified->price, 2) }}</p>
                            @if ($classified->user)
                                <p class="text-sm text-gray-500 dark:text-gray-400">Seller: {{ $classified->user->name }}</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('classifieds.inquiries.store', $classified) }}"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="message" name="message" rows="5" required
                                ...
                                placeholder="I'm interested in this listing. Is it still available?">{{ old('message') }}</textarea>
                            @error('message')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
